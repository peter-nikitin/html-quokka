...
import {
  ...
  ArgumentListContext, ArithmeticAtomContext,
  ...
  NullComparisonExpressionContext, ParenthesizedArithmeticExpressionContext,
...
export class ExpressionVisitor extends QuokkaVisitor<string | null> {
  ...
  visitVariantValueExpression = (
    ctx: VariantValueExpressionContext
  ): string | null => {
    return ctx.children.map((child) => child.accept(this)).join("");
  };

  visitRootVariantValueExpression = (
    ctx: RootVariantValueExpressionContext
  ): string | null => {
    return ctx.children.map((child) => child.accept(this)).join("");
  };

  visitVariableValueExpression = (
    ctx: VariableValueExpressionContext
  ): string | null => {
    return ctx.children.map((child) => child.accept(this)).join("");
  };

  visitMemberValueExpression = (
    ctx: MemberValueExpressionContext
  ): string | null => {
    return ctx.children.map((child) => child.accept(this)).join("");
  };
  ...
  visitStringConcatenation = (
    ctx: StringConcatenationContext
  ): string | null => {
    return ctx.children.map((child) => child.accept(this)).join("");
  };
  ...
  visitStringComparisonExpression = (
    ctx: StringComparisonExpressionContext
  ): string | null => {
    return ctx.children.map((child) => child.accept(this)).join("");
  };

  visitNullComparisonExpression = (
    ctx: NullComparisonExpressionContext
  ): string | null => {
    return ctx.children.map((child) => child.accept(this)).join("");
  };
  ...
  visitArithmeticExpression = (
    ctx: ArithmeticExpressionContext
  ): string | null => {
    return ctx.children.map((child) => child.accept(this)).join("");
  };
  ...
  visitMultiplicationExpression = (
    ctx: MultiplicationExpressionContext
  ): string | null => {
    return ctx.children.map((child) => child.accept(this)).join("");
  };

  visitMultiplicationOperand = (
    ctx: MultiplicationOperandContext
  ): string | null => {
    return ctx.children.map((child) => child.accept(this)).join("");
  };
